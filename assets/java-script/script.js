...
function showSuggestions(results) {
  ...
  results.forEach(place => {
    ...
    btn.addEventListener('click', () => {
      // when user clicks a suggestion, clear and load its weather
      clearSuggestions();
...
async function loadWeatherFor(lat, lon, placeMeta = {}) {
  ...
  try {
    ...
    // read hourly values at idx (if available)
    ...
    // weather text and icon updates
    const weatherText = weatherCodeToText(curCode);
    // update all elements that have id="weather-code" (one img and one h3 in your HTML)
    weatherElsAll.forEach(el => {
      if (el.tagName.toLowerCase() === 'img') {
        // set emoji svg as image src
        el.src = emojiToDataUri(weatherCodeToEmoji(curCode), 64);
        el.alt = weatherText;
      } else {
        // likely an h3 -> set text
    ...
    // Apply values to each tile (if present)
    tiles.forEach((tileEl, i) => setText(tileEl, tileValues[i] ?? '—'));

  } catch (err) {
    console.error('Failed to load weather:', err);
    setText(tempEl, 'Error');
    // show minimal message in tiles if error
